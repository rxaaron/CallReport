<?php
    ...
    $select=$db->query("SELECT B.HomeName, SUM(CASE WHEN A.RPh = 1 THEN 1 ELSE 0 END) AS AaronT, SUM(CASE WHEN A.RPh = 2 THEN 1 ELSE 0 END) AS AaronG, SUM(CASE WHEN A.RPh = 3 THEN 1 ELSE 0 END) AS RickL, SUM(CASE WHEN A.RPh = 4 THEN 1 ELSE 0 END) AS RobL FROM Transaction AS A INNER JOIN NursingHome AS B ON A.NursingHome = B.ID GROUP BY A.NursingHome ORDER BY B.HomeName;");
